...
class LAVAgent(AutonomousAgent):
    def sensors(self):
        ...
        # sensors.append({'type': 'sensor.camera.rgb', 'x': self._spectator.get_transform().location.x, 'y': self._spectator.get_transform().location.y, 'z': self._spectator.get_transform().location.z, 'roll': self._spectator.get_transform().rotation.roll, 'pitch': self._spectator.get_transform().rotation.pitch, 'yaw': self._spectator.get_transform().rotation.yaw,
        #     'width': 256, 'height': 288, 'fov': 64, 'id': 'BEV_RGB'})
    ...
    def flush_counterfactual_data(self, vehicle_id):

        if self.log_wandb:
            wandb.log({
                str(vehicle_id): wandb.Video(np.stack(self.counter_factual_vizs[vehicle_id]).transpose((0,3,1,2)), fps=20, format='mp4')
            })

        self.counter_factual_vizs[vehicle_id].clear()
    ...
    @torch.no_grad()
    def run_step(self, input_data, timestamp, spd, frame_num, phase = None, vehicle_id=None):
        if phase == None:
            self.num_frames += 1
            ...
            self.cmd_value = cmd.value - 1
            self.cmd_value = 3 if self.cmd_value < 0 else self.cmd_value
            
            if self.cmd_value in [4,5]:
                if self.lane_changed is not None and self.cmd_value != self.lane_changed:
                    self.lane_change_counter = 0

                self.lane_change_counter += 1
                self.lane_changed = self.cmd_value if self.lane_change_counter > {4:300,5:300}.get(self.cmd_value) else None
            else:
                self.lane_change_counter = 0
                self.lane_changed = None

            if self.cmd_value == self.lane_changed:
                self.cmd_value = 3

            # Transform to ego-coordinates
            self.wx, self.wy = _rotate(wx, wy, -imu[-1]+np.pi/2)
    ...
    def counterfactual_plan_collide(self, ego_plan_locs, other_cast_locs, other_cast_cmds, dist_threshold_static=1.0, dist_threshold_moving=2.5):
        ...
        for vehicle, (other_trajs, other_cmds) in enumerate(zip(other_cast_locs, other_cast_cmds)):
            # if vehicle == 0:
            #     print(other_trajs, other_cmds)
                ...
                for i in range(4):
                    if i == 1:
                        for epl in perturbed_ego_trajs:
                            ...
                            # print(vehicle, intersection_point)
                            if dist < dist_threshold_static:
                                if epl[1] == 1:
                                    important_vehicles.append([vehicle, [other_traj[1]], [epl[0]], epl[1], i, intersection_point])
                                else:
                                    important_vehicles.append([vehicle, [other_traj[1]], epl[0], epl[1], i, intersection_point])
                            if epl[1] == 1:
                                modded_trajectories.append([vehicle, [other_traj[1]], [epl[0]], epl[1], i, intersection_point])
                            else:
                                modded_trajectories.append([vehicle, [other_traj[1]], epl[0], epl[1], i, intersection_point])
                        continue
                    elif i == 2:
                    ...
                    for epl in perturbed_ego_trajs:
                        ...
                        if dist < dist_threshold:
                            if epl[1] == 1:
                                important_vehicles.append([vehicle, mod_traj, [epl[0]], epl[1], i, intersection_point])
                            else:
                                important_vehicles.append([vehicle, mod_traj, epl[0], epl[1], i, intersection_point])
                        if epl[1] == 1:
                            modded_trajectories.append([vehicle, mod_traj, [epl[0]], epl[1], i, intersection_point])
                        else:
                            modded_trajectories.append([vehicle, mod_traj, epl[0], epl[1], i, intersection_point])
